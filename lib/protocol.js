...
class Protocol {
    ...
    lookupModuleInstance(module_name_id, module_id, variables = null) {
        const m = this.definition.modules.filter((m) => m.id === module_name_id)[0];
        const instances = m.instances.filter((i) => i.id === module_id);
        var i;
        for (var j = 0; j < instances.length; j++)
        {
            ...
            {
                var match = true;
                for(var propertyName in instances[j].specific) {
    ...
    appendModuleInstance(name, module_name, module_id, specific)
    {
        var m = this.definition.modules.filter((m) => m.name === module_name)[0];
        const instance = {
            name: name,
            id: module_id,
            specific: specific
