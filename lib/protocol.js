...
class Protocol {
            ...
            if (item.variables && typeof item.variables[0] === "object") {
                for (const varitem of item.variables[0].variable) {
                    const variable = {
                        name: varitem.$.name,
                        type: varitem.$.type,
                        startBit: parseInt(varitem.$.startBit, 10),
                        bitLength: parseInt(varitem.$.bitLength, 10),
                        unit: varitem.$.unit,
                        values: []
                    };
    ...
    resolveDirecitonFlag(directionName) {
    ...
    lookupModuleInstance(moduleNameId, moduleId, variables = null) {
        const m = this.definition.modules.find((m) => m.id === moduleNameId);
        ...
        for (let j = 0; j < instances.length; j++) {
            if (instances[j].specific) {
                ...
                for (const propertyName of Object.keys(instances[j].specific)) {
                    ...
                    } else if (instances[j].specific[propertyName] !== variables[propertyName].value) {
                        match = false;
                    }
                }
    ...
    sendMessage(alias, variables)
    {
        console.log("Protocol - Alias: ", alias, " Variables: ", JSON.stringify(variables));
    }
    ...
    encodeFloat(bitset, startBit, bitLength, value) {
        ...
        this.encodeInt(startBit, bitLength, intValue);
    }
    ...
    encodeHexstring(bitset, startBit, bitLength, value) {
        ...
        for (let n = 0; n < bitLength; n += 4) {
            asciiValue = (value[n / 4]).toString().toUpperCase();
