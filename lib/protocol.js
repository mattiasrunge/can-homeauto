...
class Protocol extends EventEmitter {
    ...
    _decodeInt(bitset, startBit, bitLength) {
        const sign = bitset.getBits(startBit, 1, false);
        const raw = bitset.getBits(startBit, bitLength, false);
        let value = raw;
    ...
    _decodeUint(bitset, startBit, bitLength) {
        return bitset.getBits(startBit, bitLength, false);
    }
    ...
    _decodeAscii(bitset, startBit, bitLength) {
        ...
        for (let n = 0; n < bitLength; n++) {
            const code = bitset.getBits(startBit + n, 8);
